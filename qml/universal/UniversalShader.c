...
uniform sampler2D bumpFloorMap;
uniform sampler2D bumpCeilingMap;
uniform sampler2D environmentMap;

uniform lowp vec3 lightPosition;
uniform lowp vec3 lightDirection;
uniform lowp float lightIntensity;
uniform lowp float lightDistance;
uniform lowp float lightOuterAngle;
uniform lowp float lightInnerAngle;
uniform lowp vec4 lightColor;
uniform lowp float lightFlareIntensity;
uniform lowp int lightShadowSampleCount;
uniform lowp float lightAreaRadius;
uniform lowp int lightAreaSampleCount;
uniform lowp int lightIsDirectional;

uniform lowp int environmentSampleCount;

uniform lowp vec4 materialAmbientColor;
uniform lowp vec4 materialGlossyColor;

uniform lowp int computeBumps;
uniform lowp int useBumpFloorMap;
uniform lowp int useBumpCeilingMap;
uniform lowp float bumpAltitude;
uniform lowp float bumpFloorHeight;
...
uniform lowp float xRatio;
uniform lowp float yRatio;
...
float phongDiffuse()
{
    ...
    return max(dot(surfaceNormal, -lightRay), 0.0);
}
...
float orenNayarDiffuse(float roughness)
{
    ...
    float albedo = 1.0;
    vec3 lightDirection = -lightRay;

    float LdotV = dot(lightDirection, viewRay);
    float NdotL = dot(lightDirection, surfaceNormal);
    float NdotV = dot(surfaceNormal, viewRay);
...
float beckmannDiffuse(float roughness)
{
    ...
    float NdotH = dot(surfaceNormal, halfway);
...
{
    float dotViewNormal = max(dot(-viewRay, surfaceNormal), 0.0);
        ...
        )
{
    ...
    vec3 X = surfaceNormalTangent;
    vec3 Y = surfaceNormalBitangent;
    ...
    float Cdlum = .3 * Cdlin[0] + .6 * Cdlin[1]  + .1 * Cdlin[2]; // luminance approximation
...
float getFloor(vec2 position)
{
    if (useBumpFloorMap != 0)
    {
        ...
        if (sampledColor.a > 0.95)
        {
            return ((sampledColor.r + sampledColor.g + sampledColor.b) / 3.0) * bumpFloorHeight;
        }
...
float getCeiling(vec2 position)
{
    if (useBumpCeilingMap != 0)
    {
        ...
        if (sampledColor.a > 0.95)
        {
            return ((sampledColor.r + sampledColor.g + sampledColor.b) / 3.0) * bumpCeilingHeight;
        }
...
float rayMarchShadows(vec3 start, vec3 finish)
{
    ...
    for (areaIndex = 0; areaIndex < actualAreaSampleCount; areaIndex++)
    {
        ...
        else
        {
            ...
            areaPosition = finish + vec3(offsetX, offsetY, offsetZ);
#else
        ...
        vec3 position = start;
        vec3 blockPosition = start;
        float localAttenuation = 1.0;

        for (rayIndex = 0; rayIndex < lightShadowSampleCount; rayIndex++)
        {
            float floor = getFloor(position.xy);
            float ceiling = getCeiling(position.xy);

            if (ceiling == 0) floor = 0;

            if (position.z > floor && position.z < floor + ceiling)
            {
            ...
            // No need to sample outside the map
        ...
        if (localAttenuation == 0.0)
        {
            localAttenuation = distance(start, blockPosition) * lightDistance;
            localAttenuation = clamp(localAttenuation, 0.0, 1.0);
        }
...
vec4 lightUp()
{
    ...
    if (computeBumps != 0)
    {
        ...
        float s = getCeiling(position.xy + vec2(0.0,  pixelDistanceY)) * 20.0;
        float e = getCeiling(position.xy + vec2( pixelDistanceX, 0.0)) * 20.0;
        float w = getCeiling(position.xy + vec2(-pixelDistanceX, 0.0)) * 20.0;
    ...
    facing = clamp(max(dot(surfaceNormal, -viewRay), 0.0), 0.0, 1.0);
    ...
    // for (int index = 0; index < u_num_lights; index++)
    {
        ...
        if (lightDistance == 0.0 || dist < lightDistance)
        {
            ...
            if (attenuation > 0.0)
            {
                ...
                halfway = normalize(viewRay + lightRay);
...
void main()
{
    float floor = getFloor(qt_TexCoord0);
    float ceiling = getCeiling(qt_TexCoord0);

    if (ceiling == 0) floor = 0;

    position = vec3(qt_TexCoord0.x, qt_TexCoord0.y, floor + ceiling);
    surfaceNormal = up;
    surfaceNormalTangent = right;
    surfaceNormalBitangent = rear;
    viewPosition = vec3(0.5, 0.5, 10.0);
    viewRay = normalize(position - viewPosition);

    // For tests
    // float floor = getFloor(position.xy);
    // float ceiling = getCeiling(position.xy);
    // gl_FragColor = vec4(floor, 0.0, ceiling, 1.0);
    // return;
    ...
    // gl_FragColor = vec4((viewPosition.rgb + 1.0) / 2.0, 1.0);
    // gl_FragColor = vec4((viewRay.rgb + 1.0) / 2.0, 1.0);
